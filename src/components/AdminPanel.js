...
const AdminPanel = () => {
  ...
  const [generatedCode, setGeneratedCode] = useState(''); // Estado para el código generado
  const [smsStatus, setSmsStatus] = useState(''); // Estado para el estado del SMS
  ...
  const fetchData = async () => {
    try {
      if (activeSection === 'users') {
        const usersResponse = await axios.get('http://localhost:3001/api/user/usuarios', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        setUsers(usersResponse.data);
      } else if (activeSection === 'vigilantes') {
        const vigilantesResponse = await axios.get('http://localhost:3001/api/user/vigilantes', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        setVigilantes(vigilantesResponse.data);
      } else if (activeSection === 'autos') {
        const autosResponse = await axios.get('http://localhost:3001/api/user/autos', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        setAutos(autosResponse.data);
      } else if (activeSection === 'accesos') {
        const accesosResponse = await axios.get('http://localhost:3001/api/user/accesos', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        setAccesos(accesosResponse.data);
      } else if (activeSection === 'detalle_acceso') {
        const detalleResponse = await axios.get('http://localhost:3001/api/user/detalle-acceso', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        setDetalleAcceso(detalleResponse.data);
      } else if (activeSection === 'sensores') {
        const sensoresResponse = await axios.get('http://localhost:3001/api/user/sensores', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  // Función para generar el código y enviar SMS con número de teléfono manual
  const handleGenerateCode = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/api/user/access/generate-code', { phoneNumber }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
      setGeneratedCode(response.data.code || ''); // Asegúrate de que code esté definido
      setSmsStatus(response.data.message || 'Código generado con éxito');
      alert(`Código generado: ${response.data.code}`);
    } catch (error) {
      console.error('Error al generar el código o enviar SMS:', error);
      if (error.response && error.response.data) {
        setGeneratedCode(error.response.data.code || '');
        setSmsStatus(error.response.data.message || 'Error al generar el código o enviar SMS');
      } else {
        setSmsStatus('Error al generar el código o enviar SMS');
      }
      alert('Error al generar el código o enviar SMS');
    }
  };
  ...
  const handleUpdateUser = async (id) => {
    try {
      await axios.put(`http://localhost:3001/api/user/usuarios/${id}`, editUser, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDeleteUser = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este usuario?')) {
      try {
        await axios.delete(`http://localhost:3001/api/user/usuarios/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  const handleExcelUpload = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        const response = await axios.post('http://localhost:3001/api/user/import-users', jsonData, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        ...
        alert('Error al importar usuarios desde Excel: ' + (error.response ? error.response.data.message : 'Revisa la consola'));
      }
    };
    reader.readAsArrayBuffer(excelFile);
  };

  // Funciones para vigilantes
  const handleInsertVigilante = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/api/user/vigilantes', newVigilante, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUpdateVigilante = async (id) => {
    try {
      await axios.put(`http://localhost:3001/api/user/vigilantes/${id}`, editVigilante, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDeleteVigilante = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este vigilante?')) {
      try {
        await axios.delete(`http://localhost:3001/api/user/vigilantes/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  const handleInsertAuto = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/api/user/autos', newAuto, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUpdateAuto = async (id) => {
    try {
      await axios.put(`http://localhost:3001/api/user/autos/${id}`, editAuto, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDeleteAuto = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este auto?')) {
      try {
        await axios.delete(`http://localhost:3001/api/user/autos/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  const handleInsertAcceso = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/api/user/accesos', newAcceso, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUpdateAcceso = async (id) => {
    try {
      await axios.put(`http://localhost:3001/api/user/accesos/${id}`, editAcceso, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDeleteAcceso = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este acceso?')) {
      try {
        await axios.delete(`http://localhost:3001/api/user/accesos/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  // Funciones para detalle_acceso
  const handleInsertDetalleAcceso = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/api/user/detalle-acceso', newDetalleAcceso, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUpdateDetalleAcceso = async (id) => {
    try {
      await axios.put(`http://localhost:3001/api/user/detalle-acceso/${id}`, editDetalleAcceso, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDeleteDetalleAcceso = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este registro de acceso?')) {
      try {
        await axios.delete(`http://localhost:3001/api/user/detalle-acceso/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  const handleInsertSensor = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/api/user/sensores', newSensor, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUpdateSensor = async (id) => {
    try {
      await axios.put(`http://localhost:3001/api/user/sensores/${id}`, editSensor, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDeleteSensor = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este sensor?')) {
      try {
        await axios.delete(`http://localhost:3001/api/user/sensores/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
      ...
      <div className={`sidebar ${isSidebarOpen ? 'open' : 'closed'}`}>
        ...
        <ul className="sidebar-menu">
          ...
          <li>
            <button
              onClick={() => {
                setActiveSection('vigilantes');
                setIsSidebarOpen(false);
              }}
              className={activeSection === 'vigilantes' ? 'active' : ''}
            >
          ...
          <li>
            <button
              ...
              className={activeSection === 'autos' ? 'active' : ''}
            >
          ...
          <li>
            <button
              ...
              className={activeSection === 'accesos' ? 'active' : ''}
            >
          ...
          <li>
            <button
              onClick={() => {
                setActiveSection('detalle_acceso');
                setIsSidebarOpen(false);
              }}
              className={activeSection === 'detalle_acceso' ? 'active' : ''}
            >
          ...
          <li>
            <button
              onClick={() => {
                setActiveSection('sensores');
                setIsSidebarOpen(false);
              }}
              className={activeSection === 'sensores' ? 'active' : ''}
            >
          ...
          <li>
            <button
              onClick={() => {
                setActiveSection('generate-code');
                setIsSidebarOpen(false);
              }}
              className={activeSection === 'generate-code' ? 'active' : ''}
            >
              <i className="fas fa-qrcode"></i> Generar Código
            </button>
          </li>
          <li>
            <button
      ...
      <div className={`main-content ${isSidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
        <div className="content-header">
          <h1>
            ...
            {activeSection === 'generate-code' && 'Generar Código de Acceso'}
          </h1>
        </div>
            ...
            <div className="form-column">
              ...
              {activeSection === 'generate-code' && (
                <div className="form-section">
                  ...
                  <button onClick={handleGenerateCode} className="btn-submit">
                    Generar y Enviar Código
                  </button>
                  {generatedCode && (
                    <div className="generated-code">
                      <p>Código Generado: <strong>{generatedCode}</strong></p>
                    </div>
                  )}
                  {smsStatus && (
                    ...
                    {users.map((user) => (
                      <tr key={user.id_usuario}>
                        ...
                        <td>
                          {editUser && editUser.id_usuario === user.id_usuario ? (
                            <>
                              <button onClick={() => handleUpdateUser(user.id_usuario)} className="btn-action">
                                Guardar
                              </button>
                              <button onClick={() => setEditUser(null)} className="btn-action">
                                Cancelar
                              </button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => setEditUser(user)} className="btn-action">
                                Editar
                              </button>
                              <button onClick={() => handleDeleteUser(user.id_usuario)} className="btn-action">
                                Eliminar
                              </button>
                            </>
                          )}
                      ...
                      <tr key={vigilante.id_vigilante}>
                        ...
                        <td>
                          {editVigilante && editVigilante.id_vigilante === vigilante.id_vigilante ? (
                            <>
                              <button
                                onClick={() => handleUpdateVigilante(vigilante.id_vigilante)}
                                className="btn-action"
                              >
                                Guardar
                              </button>
                              <button onClick={() => setEditVigilante(null)} className="btn-action">
                                Cancelar
                              </button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => setEditVigilante(vigilante)} className="btn-action">
                                Editar
                              </button>
                              <button
                                onClick={() => handleDeleteVigilante(vigilante.id_vigilante)}
                                className="btn-action"
                              >
                                Eliminar
                              </button>
                    ...
                    {autos.map((auto) => (
                      <tr key={auto.id_auto}>
                        ...
                        <td>
                          {editAuto && editAuto.id_auto === auto.id_auto ? (
                            <>
                              <button onClick={() => handleUpdateAuto(auto.id_auto)} className="btn-action">
                                Guardar
                              </button>
                              <button onClick={() => setEditAuto(null)} className="btn-action">
                                Cancelar
                              </button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => setEditAuto(auto)} className="btn-action">
                                Editar
                              </button>
                              <button onClick={() => handleDeleteAuto(auto.id_auto)} className="btn-action">
                                Eliminar
                              </button>
                            </>
                          )}
                    ...
                    {accesos.map((acceso) => (
                      <tr key={acceso.id_acceso}>
                        ...
                        <td>
                          {editAcceso && editAcceso.id_acceso === acceso.id_acceso ? (
                            <>
                              <button onClick={() => handleUpdateAcceso(acceso.id_acceso)} className="btn-action">
                                Guardar
                              </button>
                              <button onClick={() => setEditAcceso(null)} className="btn-action">
                                Cancelar
                              </button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => setEditAcceso(acceso)} className="btn-action">
                                Editar
                              </button>
                              <button onClick={() => handleDeleteAcceso(acceso.id_acceso)} className="btn-action">
                                Eliminar
                              </button>
                            </>
                          )}
                      ...
                      <tr key={detalle.id_detalle_acceso}>
                        ...
                        <td>
                          {editDetalleAcceso && editDetalleAcceso.id_detalle_acceso === detalle.id_detalle_acceso ? (
                            <>
                              <button
                                onClick={() => handleUpdateDetalleAcceso(detalle.id_detalle_acceso)}
                                className="btn-action"
                              >
                                Guardar
                              </button>
                              <button onClick={() => setEditDetalleAcceso(null)} className="btn-action">
                                Cancelar
                              </button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => setEditDetalleAcceso(detalle)} className="btn-action">
                                Editar
                              </button>
                              <button
                                onClick={() => handleDeleteDetalleAcceso(detalle.id_detalle_acceso)}
                                className="btn-action"
                              >
                                Eliminar
                              </button>
                    ...
                    {sensores.map((sensor) => (
                      <tr key={sensor.id_sensor}>
                        ...
                        <td>
                          {editSensor && editSensor.id_sensor === sensor.id_sensor ? (
                            <>
                              <button onClick={() => handleUpdateSensor(sensor.id_sensor)} className="btn-action">
                                Guardar
                              </button>
                              <button onClick={() => setEditSensor(null)} className="btn-action">
                                Cancelar
                              </button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => setEditSensor(sensor)} className="btn-action">
                                Editar
                              </button>
                              <button onClick={() => handleDeleteSensor(sensor.id_sensor)} className="btn-action">
                                Eliminar
                              </button>
                            </>
                          )}
