...
const ResidentPanel = () => {
  ...
  useEffect(() => {
    ...
    if (!token || localStorage.getItem('roleName') !== 'Residente') {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeSection === 'dashboard' || activeSection === 'access') {
        try {
          const accessResponse = await axios.get('http://localhost:3001/api/resident/access', {
      ...
      if (activeSection === 'dashboard' || activeSection === 'cars' || activeSection === 'access-codes') {
        try {
          const carsResponse = await axios.get('http://localhost:3001/api/resident/cars', {
            headers: { Authorization: `Bearer ${token}` },
          });
      ...
      if (activeSection === 'dashboard' || activeSection === 'access-codes') {
        try {
          const codesResponse = await axios.get('http://localhost:3001/api/resident/access-codes', {
            headers: { Authorization: `Bearer ${token}` },
          });
        ...
        try {
          const sensorsResponse = await axios.get('http://localhost:3001/api/resident/sensors', {
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  const handleGenerateCode = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://localhost:3001/api/resident/generate-code',
        {
          id_auto: selectedCarId, // Usar el auto seleccionado (puede ser null)
  ...
  const handleInsertCar = async (e) => {
    ...
    setError(null);
    try {
      await axios.post('http://localhost:3001/api/resident/cars', newCar, {
  ...
  const handleUpdateCar = async (id) => {
    ...
    setError(null);
    try {
      await axios.put(`http://localhost:3001/api/resident/cars/${id}`, editCar, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleDeleteCar = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este auto?')) {
      ...
      setError(null);
      try {
        await axios.delete(`http://localhost:3001/api/resident/cars/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  const handleDeleteAccessCode = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este código de acceso?')) {
      ...
      setError(null);
      try {
        await axios.delete(`http://localhost:3001/api/resident/access-codes/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
    ...
    <div className="admin-panel">
      ...
      <div className={`main-content ${isSidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
        <div className="content-header">
          <h1>
            {activeSection === 'dashboard' && 'Dashboard Residente'}
